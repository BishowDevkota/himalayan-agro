...
export default function AdminDistributorsClient({ initialVendors = [] }: { initialVendors?: any[] }) {
  const [distributors, setVendors] = useState(initialVendors || []);
  ...
  const fetchVendors = useCallback(async (opts: { status?: string; q?: string } = {}) => {
    setLoading(true);
    try {
      const url = new URL("/api/admin/distributors", window.location.origin);
      ...
      if (!res.ok) throw new Error(json?.message || "Failed to load distributors");
      setVendors(json.distributors || []);
    } catch (err: any) {
      ...
      <div className="bg-white border border-slate-200/60 rounded-2xl p-4 sm:p-5 shadow-sm">
        ...
        >
          ...
          <div className="text-sm text-slate-500">
            Showing <span className="font-semibold text-slate-700">{distributors.length}</span> distributors
          </div>
        </form>
      </div>
        ...
        <table className="w-full text-sm text-slate-800">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!loading && distributors.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-slate-400">No distributor requests.</td></tr>
            )}
            {!loading && distributors.map((v: any) => (
