...
export default function DistributorProductForm({ initial = null }: { initial?: any }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const url = initial ? `/api/distributor/products/${initial._id}` : `/api/distributor/products`;
      ...
      <div className="lg:col-span-2 bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Add title, description and pricing information.</p>
          </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4">
          <div>
            ...
            <input
              className="mt-1 block w-full rounded-lg border border-gray-200 bg-white px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-200 text-slate-900"
              value={name}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input className="mt-1 block w-full rounded-lg border border-gray-200 px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-200 text-slate-900" value={brand} onChange={(e) => setBrand(e.target.value)} />
            </div>

            <div>
              <label className="block text-sm font-medium">Category</label>
              <select value={selectedCategoryId} onChange={(e) => { setSelectedCategoryId(e.target.value); }} className="mt-1 block w-full rounded-lg border border-gray-200 px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-200">
          ...
          <div>
            ...
            <textarea className="mt-1 block w-full rounded-lg border border-gray-200 px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-200 text-slate-800" value={description} onChange={(e) => setDescription(e.target.value)} rows={4} />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <input type="number" step="0.01" className="pl-8 pr-3 w-full rounded-lg border border-gray-200 px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-200" value={price} onChange={(e) => setPrice(Number(e.target.value))} required />
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium">Stock <span className="text-rose-600">*</span></label>
              <input type="number" className="mt-1 block w-full rounded-lg border border-gray-200 px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-200" value={stock} onChange={(e) => setStock(Number(e.target.value))} required />
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <button type="submit" className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-white shadow-sm hover:bg-emerald-700" disabled={loading}>{loading ? "Saving…" : "Save product"}</button>
