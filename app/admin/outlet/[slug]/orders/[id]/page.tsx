...
export default async function OutletOrderDetailPage({ params }: { params: { slug: string; id: string } } | { params: Promise<{ slug: string; id: string }> }) {
  ...
  const allowed = await orderBelongsToOutlet(order, String(outlet._id));
      ...
      <div className="max-w-7xl mx-auto py-16 px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <a href={`/admin/outlet-${slug}/orders`} className="rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm text-slate-700">← Back to orders</a>
          </div>
        </div>
